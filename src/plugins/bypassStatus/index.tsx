...
async function showNotification(message: Message, guildId: string | undefined): Promise<void> {
    try {
        ...
        content = content.replace(channelRegex, (_match: any, channelId: string) => {
            return `#${ChannelStore.getChannel(channelId)?.name}`;
        });

        content = content.replace(userRegex, (_match: any, userId: string) => {
            return `@${(UserStore.getUser(userId) as any)?.globalName ?? userId}`;
        });

        await Notifications.showNotification({
            title: `${(message.author as any).globalName} ${guildId ? `(#${channel?.name})` : ""}`,
            body: content,
...
const settings = definePluginSettings({
    ...
    allowOutsideOfDms: {
        ...
        description: "Allow selected users to bypass status outside of DMs too",
        default: false
    },
    respectSilentPings: {
        type: OptionType.BOOLEAN,
...
export default definePlugin({
    name: "BypassStatus",
    description: "Still get notifications from specific sources when in do not disturb mode. Right-click on users/channels/guilds to set them to bypass.",
